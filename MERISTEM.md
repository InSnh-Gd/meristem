...
MERISTEM 是一个 TypeScript-first、Elysia-first、Eden-first 但非 Eden-only、微服务优先的 Monorepo 分布式 M 网络控制系统。
...
用户首先看到的是：M 网络的当前状态，包括 Core Node 健康状态、Stem / Leaf 节点概览、关键服务状态、最近 Timeline Log、需要关注的策略或审计事件。

这一刻的设计目标是：操作者无需理解全部内部实现，也能立即判断系统是否安全、是否降级、哪里需要处理。

### 2.2 唯一的核心交互

整个产品只有一个真正的交互：**在明确契约、权限和审计边界下，对 M 网络中的节点、服务、配置和高风险操作做可追踪控制**。
...
- ❌ 不用“智能地自动处理一切”这类无法审计的表达。
- ❌ 不把 LLM 描述为最终决策者或事实来源。
- ❌ 不用“插件生态”替代 M-Extension 的补充定位。
- ❌ 不把微服务描述成独立割裂的系统边界。
...
以下特性或设计方向明确不在产品范围内。提出这些通常意味着对产品意图的误读：

- 🚫 不把 Core 做成承载所有业务逻辑的大单体。
- 🚫 不把 M-Extension 当成主要功能承载层或插件优先生态。
- 🚫 不默认引入 GraphQL、Temporal、Tekton、Raft、Service Mesh、gRPC everywhere。
- 🚫 不采用每服务独立数据库作为默认架构。
- 🚫 不把 OpenSearch、NATS KV、M-EventBus、Yjs 当作权威状态源。
- 🚫 不让 Leaf Node 默认拥有高权限、广 API 或广互联范围。
- 🚫 不让 LLM 绕过 M-Policy、替代 Audit Log 或直接执行高权限操作。
- 🚫 不把 APISIX、Redis / KeyDB、Wasm / Zig 作为默认基础依赖。
- 🚫 不为了抽象而抽象，不用复杂框架掩盖契约不清。
...
| 日期 | 版本 | 变更内容 | 变更人 |
|------|------|---------|--------|
| 2026-05-04 | v0.1 | 从 `meristem_v_next_developer_document_v_0_1.md` 抽取产品意图初稿 | Codex |
