...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      from: {
        // Bun 将依赖统一安装在根 node_modules 并为每个子包维护 .bun 存储；dependency-cruiser 从根 package.json
        // 读取时无法识别子 workspace 的 dependencies，导致所有子包依赖被误判。该问题已知，暂不对源码启用此规则。
        pathNot: '.*'
      },
      to: {
        dependencyTypes: [
    ...
    },
    {
      ...
      severity: 'error',
      from: {
        // SvelteKit 虚拟别名（$lib/$app）和生成文件在静态解析下不可达，不由本规则管辖
        pathNot: '^apps/m-ui/'
      },
      to: {
    ...
    {
      name: 'no-apps-importing-service-app-internals',
      comment:
    ...
    },
    {
      ...
      comment:
        ...
        'Apps are entry points; reusable logic belongs in packages.',
      severity: 'error',
      from: {
    ...
    },
    {
      ...
      from: {
        path: '^()',
        // M-UI 使用 Svelte/Vite 作为其构建与运行时框架；这些依赖声明在 apps/m-ui/package.json 的 dependencies 中，
        // 但 dependency-cruiser 目前从仓库根 package.json 读取，会误判为 devDependencies。
        pathNot: '[.](?:spec|test)[.](?:js|mjs|cjs|jsx|ts|mts|cts|tsx)$|^apps/m-ui/'
      },
      to: {
        dependencyTypes: [
          'npm-dev',
  ...
  options: {
    ...
    enhancedResolveOptions: {
      ...
      extensions: [".ts", ".tsx", ".svelte", ".mjs", ".js", ".d.ts"],
      
      // What to consider a 'main' field in package.json
      mainFields: ["module", "main", "types", "typings"],
      
      // Meristem 使用 SvelteKit 的 $lib/$app 虚拟别名和 Bun workspace 相对路径；
      // dependency-cruiser 不解析这些别名，相关不可解析问题由 not-to-unresolvable 规则排除。
      //
...
// generated: dependency-cruiser@17.4.3 on 2026-06-18T13:06:41.881Z
