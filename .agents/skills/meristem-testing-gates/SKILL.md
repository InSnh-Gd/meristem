...
Detailed review tables (Type Safety & Route Architecture Review Checklist, M-UI Ownership Review Checklist, M-UI Frontend Review Gates) and the `review-work` / `scrutinize` integration instructions live in [references/review-checklists.md](references/review-checklists.md). Load that file when reviewing or claiming completion for type-safety, route architecture, M-UI, SDUI, BFF, CommandWell, or extension-UI-adjacent work: a review verdict must be FAIL if any Hard Gate item in the relevant checklist is violated, regardless of other quality dimensions.
...
- **[review-checklists.md](references/review-checklists.md)**: Type Safety & Route Architecture Review Checklist, M-UI Ownership Review Checklist, M-UI Frontend Review Gates, and Review Stage Integration (`review-work` Agent 3 prompt injection, `scrutinize` diff checks).
