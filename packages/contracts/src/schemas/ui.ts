...
export const BffOperationalRuntimeTruthSchema = Schema.Struct({
  auth: Schema.Struct({
    mode: Schema.Literals(['oidc', 'local-dev']),
    summary: Schema.String,
    stateSource: StateSourceMetadataSchema
  }),
  secretProvider: Schema.Struct({
    status: Schema.Literals(['resolved', 'missing', 'denied']),
    summary: Schema.String,
    stateSource: StateSourceMetadataSchema
  }),
  netbirdProcess: Schema.Struct({
    desired: Schema.Literals(['running', 'stopped', 'unknown']),
    observed: Schema.Literals(['running', 'stopped', 'not-run', 'unknown']),
  ...
  profile: Schema.Struct({
    state: Schema.Literals(['enabled', 'disabled', 'degraded']),
    reason: Schema.optional(Schema.String),
    stateSource: StateSourceMetadataSchema
  }),
