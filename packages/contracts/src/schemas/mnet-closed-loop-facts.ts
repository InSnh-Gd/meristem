...
export const MNetClosedLoopStateSourceSchema = Schema.Literal(
...
export const MNetNodeKindSchema = Schema.Literal('core', 'stem', 'leaf')
export type MNetNodeKindFromSchema = typeof MNetNodeKindSchema.Type

export const MNetNodeRuntimeStateSchema = Schema.Literal(
  'joining',
  'healthy',
  'degraded',
  'offline',
...
export const MNetJoinCredentialStatusSchema = Schema.Literal(
  'pending',
  'issued',
  'active',
  'rotating',
...
export const MNetMapFreshnessSchema = Schema.Literal('fresh', 'stale', 'expired', 'fail_closed')
...
export const MNetSignedTopologyMapStatusSchema = Schema.Struct({
  ...
  validation: Schema.Literal('valid', 'signature_invalid', 'stale', 'expired')
})
...
export const MNetKeyRegistrationStatusSchema = Schema.Struct({
  ...
  status: Schema.Literal('registered', 'rotation_required', 'revoked', 'stale'),
...
export const MNetTunnelModeSchema = Schema.Literal('direct', 'relay', 'forced-relay', 'none')
...
export const MNetTunnelHealthSchema = Schema.Struct({
  ...
  status: Schema.Literal('up', 'degraded', 'down'),
...
export const MNetSidecarDegradedReasonSchema = Schema.Literal(
  'expired_credentials',
  'missing_signal',
  'missing_relay',
  'missing_stun',
...
export const MNetSidecarStatusSchema = Schema.Struct({
  ...
  proofPath: Schema.Literal('sidecar-proof', 'runtime-probe', 'operator-report'),
  ...
  Schema.filter(value => {
    ...
    if (value.fallbackTransport && !degraded)
      issues.push(
        issue(['fallbackTransport'], 'fallback transport is only valid for degraded sidecar status')
      )
    return issues
  })
)
...
export const MNetRelayPolicyStateSchema = Schema.Literal('enabled', 'disabled', 'denied')
...
export const MNetTopologyViewNetworkSchema = Schema.Struct({
  ...
  profileVersion: MNetProfileV03VersionSchema,
  status: Schema.Literal('healthy', 'degraded', 'fail_closed', 'migration_required'),
