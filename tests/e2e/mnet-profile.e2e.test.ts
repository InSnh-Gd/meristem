...
async function mnetFetch(path: string, token: string, init?: RequestInit): Promise<{ ok: boolean; status: number; data: unknown }> {
...
async function policyFetch(path: string, token: string, init?: RequestInit): Promise<{ ok: boolean; status: number; data: unknown }> {
...
describe('e2e: m-net profile lifecycle', () => {
  ...
  test.skipIf(!infraOk)('runs full enable->approve->disable lifecycle and checks event/audit/timeline consistency', async () => {
    ...
    const targetApproval = approvalsBody.approvals.find((item) => item.operationId === enableBody.operationId)
    expect(targetApproval).toBeDefined()
    expect(targetApproval?.originService).toBe('m-net')
    expect(targetApproval?.status).toBe('pending')

    const approve = await policyFetch(`/api/v0/policy/approvals/${targetApproval!.id}/approve`, securityAdminToken, {
      method: 'POST',
    ...
    const timelineEntries = (timeline.data as { entries: Array<{ correlationId?: string; subject?: string }> }).entries
    expect(timelineEntries.some((entry) => entry.correlationId === correlationId && entry.subject === 'mnet.profile.enabled')).toBe(true)
