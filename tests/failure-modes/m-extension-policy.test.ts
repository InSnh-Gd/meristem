...
import type { MExtensionApp } from '../../services/m-extension/src/public-types.ts'
...
function deniedDeps(fullLogs: unknown[]): MExtensionDeps {
  return {
    ...
    log: {
      async writeTimeline() {},
      async writeFull(_level, message, _correlationId, payload) {
        fullLogs.push({ message, payload })
      },
      async writeAudit() {}
    },
...
function allowDeps(
  ...
  return {
    ...
    policy: {
      async authorize() {
        return { result: 'allow', id: 'decision-allow', reasons: [] }
      }
    },
    log: {
      async writeTimeline() {},
      ...
      async writeAudit(actor, action, resource, result, correlationId, payload) {
        audits.push({ actor, action, resource, result, correlationId, payload })
      }
    },
    events: {
...
async function register(app: MExtensionApp, token: string, next: unknown = manifest) {
  return app.handle(
    new Request(`http://localhost${mExtensionApiRoutes.register}`, {
      method: 'POST',
...
describe('M-Extension failure modes', () => {
  it('policy denial fails closed before registration persistence', async () => {
    ...
    const list = await app.handle(
      new Request(`http://localhost${mExtensionApiRoutes.collection}`, {
  ...
  it('manifest validation rejection normalizes invalid kind in rejected lifecycle payload', async () => {
    ...
    const response = await register(
      app,
      token,
      manifestWith({ id: 'extension-invalid-kind', kind: 'script' })
    )
    ...
    expect(events).toEqual([
      {
        ...
        payload: expect.objectContaining({ kind: 'metadata-only' })
      }
    ])
  })
  ...
  it('enable and disable policy denial write Audit before returning 403', async () => {
    ...
    const enable = await app.handle(
      new Request(`http://localhost${mExtensionApiRoutes.collection}/${manifest.id}/enable`, {
        method: 'POST',
        headers: { authorization: `Bearer ${denyToken}`, 'content-type': 'application/json' },
        body: JSON.stringify({})
      })
    )
    const disable = await app.handle(
      new Request(`http://localhost${mExtensionApiRoutes.collection}/${manifest.id}/disable`, {
        method: 'POST',
        headers: { authorization: `Bearer ${denyToken}`, 'content-type': 'application/json' },
        body: JSON.stringify({})
      })
  ...
  it('audit failure fails closed before register, enable, and disable mutations', async () => {
    ...
    const registerResponse = await register(
      registerApp,
      token,
      manifestWith({ id: 'extension-audit-register' })
    )
    ...
    const setupApp = createMExtensionApp(allowDeps([], [], [], store))
    expect(
      (await register(setupApp, token, manifestWith({ id: 'extension-audit-control' }))).status
    ).toBe(200)
    const controlApp = createMExtensionApp({ ...allowDeps([], [], [], store), log: failingLog })
    const enable = await controlApp.handle(
      new Request(
        `http://localhost${mExtensionApiRoutes.collection}/extension-audit-control/enable`,
        {
          method: 'POST',
          headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
          body: JSON.stringify({})
    ...
    const disable = await controlApp.handle(
      new Request(
        `http://localhost${mExtensionApiRoutes.collection}/extension-audit-control/disable`,
        {
          method: 'POST',
          headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
          body: JSON.stringify({})
  ...
  it('enable and disable event publication failures emit failure lifecycle events', async () => {
    ...
    const token = await mintLocalToken({ actor: 'admin', secret: 'm-extension-allow-secret' })
    expect(
      (await register(setupApp, token, manifestWith({ id: 'extension-event-control' }))).status
    ).toBe(200)
    ...
    const enable = await app.handle(
      new Request(
        `http://localhost${mExtensionApiRoutes.collection}/extension-event-control/enable`,
        {
          method: 'POST',
          headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
          body: JSON.stringify({})
    ...
    const disable = await app.handle(
      new Request(
        `http://localhost${mExtensionApiRoutes.collection}/extension-event-control/disable`,
        {
          method: 'POST',
          headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
          body: JSON.stringify({})
  ...
  it('store dependency failure returns stable 503 envelopes', async () => {
    ...
    const show = await app.handle(
      new Request(`http://localhost${mExtensionApiRoutes.collection}/${manifest.id}`, {
        headers: { authorization: `Bearer ${token}` }
      })
    )
    const registerResponse = await register(
      app,
      token,
      manifestWith({ id: 'extension-store-down' })
    )
