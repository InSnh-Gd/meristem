...
describe('Approval execution failure modes', () => {
  ...
  it('event publish failure does not create false authoritative state', async () => {
    ...
    deps.events.publish = async (subject, event) =>
      subject === 'task.operation.suspended.v0'
        ? {
            ok: false as const,
            error: { code: 'event.publish_failed', message: 'NATS unavailable' }
          }
  ...
  it('consumed idempotency key cannot be resumed twice', async () => {
    ...
    const first = await app.handle(
      new Request(`http://localhost/internal/v0/task-operations/${suspendedOp.id}/resume`, {
        method: 'POST',
        headers: { 'x-meristem-internal-token': process.env.MERISTEM_INTERNAL_TOKEN }
      })
    )
  ...
  it('M-Task reject endpoint transitions suspended op to rejected', async () => {
    ...
    const response = await app.handle(
      new Request(`http://localhost/internal/v0/task-operations/${suspendedOp.id}/reject`, {
        method: 'POST',
        headers: { 'x-meristem-internal-token': process.env.MERISTEM_INTERNAL_TOKEN }
      })
    )
  ...
  it('M-Task reject endpoint rejects missing internal token', async () => {
    ...
    const response = await app.handle(
      new Request('http://localhost/internal/v0/task-operations/fake-id/reject', {
        method: 'POST',
        headers: {}
      })
    )
