...
import { extensionPermission, mExtensionManifestVersion, type MExtensionManifestV01 } from '../../packages/contracts/src/index.ts'
...
describe('extension CLI commands', () => {
  it('routes list/show/enable/disable to M-Extension client methods', async () => {
    ...
    const cli = createCliRunner({
      ...
      async listExtensions() { calls.push('extension:list'); return { extensions: [] } },
      async getExtension(id) { calls.push(`extension:show:${id}`); return { definition: { id } } as never },
      async enableExtension(id) { calls.push(`extension:enable:${id}`); return { instance: { status: 'enabled' } } as never },
      async disableExtension(id) { calls.push(`extension:disable:${id}`); return { instance: { status: 'disabled' } } as never }
    })

    expect((await cli.run(['extension', 'list'])).exitCode).toBe(0)
    expect((await cli.run(['extension', 'show', 'extension-cli-demo'])).stdout).toContain('extension-cli-demo')
    expect((await cli.run(['extension', 'enable', 'extension-cli-demo'])).stdout).toContain('enabled')
    expect((await cli.run(['extension', 'disable', 'extension-cli-demo'])).stdout).toContain('disabled')
  ...
  it('missing extension command args return usage errors', async () => {
    const cli = createCliRunner({ async status() { throw new Error('should not be called') } })

    expect((await cli.run(['extension', 'show'])).stderr).toContain('usage: meristem extension show <id>')
    expect((await cli.run(['extension', 'register'])).stderr).toContain('usage: meristem extension register <manifest-file>')
    expect((await cli.run(['extension', 'enable'])).stderr).toContain('usage: meristem extension enable <id>')
    expect((await cli.run(['extension', 'disable'])).stderr).toContain('usage: meristem extension disable <id>')
  })
