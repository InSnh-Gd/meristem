...
/** 需要排除的目录名（不进入扫描） */
const SKIP_DIR_NAMES = new Set(['node_modules', '.git', '.codegraph', '.omo', '.agents'])
...
describe('evidence path standards guard', () => {
  ...
  it('no .omo/ references in source, test, or doc files', () => {
    const omoViolations = violations.filter(v => v.reason.includes('.omo/'))

    if (omoViolations.length > 0) {
      const report = omoViolations.map(v => `  ${v.file}:${v.line}: ${v.content}`).join('\n')
      expect().fail(`发现 ${omoViolations.length} 处 ".omo/" 内部编排路径引用:\n${report}`)
    } else {
      // 断言通过：没有违规
  ...
  it('no task-number-prefixed evidence names in docs/ and tests/', () => {
    const taskPrefixViolations = violations.filter(v => v.reason.includes('任务编号前缀'))

    if (taskPrefixViolations.length > 0) {
      const report = taskPrefixViolations.map(v => `  ${v.file}:${v.line}: ${v.content}`).join('\n')
      expect().fail(`发现 ${taskPrefixViolations.length} 处任务编号前缀证据文件名:\n${report}`)
    } else {
  ...
  it('all test-code evidence paths derive from import.meta.dir or mkdtemp', () => {
    ...
    for (const f of testFiles) {
      ...
      if (content.includes('tests/evidence')) {
        const hasMetaDir =
          content.includes('import.meta.dir') || content.includes('import.meta.url')
