...
describe('M-Task service cutover', () => {
  ...
  it('lists, reads, and cancels queued M-Task requests', async () => {
    ...
    const read = await app.handle(
      new Request(`http://localhost/api/v0/tasks/${submitted.task.id}`, {
        headers: { authorization: 'Bearer operator-token' }
      })
    )
    ...
    const cancel = await app.handle(
      new Request(`http://localhost/api/v0/tasks/${submitted.task.id}/cancel`, {
        method: 'POST',
        headers: { authorization: 'Bearer operator-token' }
      })
    )
    expect(cancel.status).toBe(200)
    const canceled = (await cancel.json()) as {
      task: { status: string }
