...
describe('Approval schema and RBAC drift', () => {
  ...
  it('approval, vote, and suspended operation schemas decode contract records', async () => {
    expect(Schema.decodeUnknownSync(PolicyApprovalSchema)({
      id: 'test-id',
    ...
    expect(Schema.decodeUnknownSync(PolicyApprovalVoteSchema)({
      id: 'vote-id',
  ...
  it('Elysia approval schemas match contract type shapes', async () => {
    ...
    for (const key of ['id', 'policyDecisionId', 'originService', 'operationId', 'requestedBy', 'requiredAction', 'status', 'quorumRequired', 'expiresAt', 'createdAt', 'updatedAt']) {
