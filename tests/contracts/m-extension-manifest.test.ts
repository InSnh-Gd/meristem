...
import { extensionPermission, extensionPermissions, permissions } from '../../packages/contracts/src/literals.ts'
import { MExtensionEventSubjectSchema, MExtensionLifecyclePayloadSchema, MExtensionManifestV01Schema } from '../../packages/contracts/src/schemas/extension.ts'
import { mExtensionEventSubjects, mExtensionManifestVersion, mExtensionScope } from '../../packages/contracts/src/types/extension.ts'
...
describe('M-Extension manifest contracts', () => {
  ...
  it('rejects unknown permissions and high risk manifests before registration', () => {
    const unknownPermission = validateExtensionManifest({ ...manifest, requestedPermissions: ['dynamic:grant'] })
    expect(unknownPermission.ok).toBe(false)
    if (!unknownPermission.ok) expect(unknownPermission.code).toBe('extension.manifest.unknown_permission')
  ...
  it('rejects unsafe manifest identifiers and nested executable or secret payloads', () => {
    ...
    const nestedSecret = validateExtensionManifest({ ...manifest, futureResourceLimits: { nested: { webhookToken: 'secret' } } })
    expect(nestedSecret.ok).toBe(false)
    if (!nestedSecret.ok) expect(nestedSecret.code).toBe('extension.manifest.executable_payload')

    const futureEntrypoint = validateExtensionManifest({ ...manifest, futureEntrypoint: 'https://example.invalid/hook' })
  ...
  it('validates lifecycle event subjects and payloads', () => {
    expect(Schema.decodeUnknownSync(MExtensionEventSubjectSchema)(mExtensionEventSubjects.instanceEnabled)).toBe(mExtensionEventSubjects.instanceEnabled)
