...
describe.skipIf(!containerRuntimeAvailable)('Keycloak dev realm contract', () => {
  test(
    'exposes discovery/JWKS and mints deterministic tokens for operator, viewer, and admin actors',
    async () => {
      ...
      try {
        ...
        for (const actor of Object.keys(expectedActors) as Array<keyof typeof expectedActors>) {
          ...
          const tokenPreferredUsername = typeof decoded.preferred_username === 'string' ? decoded.preferred_username : ''
          expect([expectedActors[actor].displayName, actorLoginName]).toContain(tokenPreferredUsername)
          ...
          expect(realmAccess?.roles).toEqual(
            expect.arrayContaining(expectedActors[actor].realmRoles)
          )
          ...
          expect([expectedActors[actor].displayName, actorLoginName]).toContain(session.actor.displayName)
          ...
          expect(session.permissions).toEqual(
            expect.arrayContaining(expectedActors[actor].realmRoles.filter(role => role.includes(':')))
          )
        }
      } finally {
        await fixture.stop()
