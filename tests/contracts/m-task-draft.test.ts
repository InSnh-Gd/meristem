...
describe('M-Task service alignment', () => {
  it('records the accepted M-Task cutover and implementation baseline', async () => {
    ...
    expect(mTaskDraftDoc).toContain('`meristem task assign` is not retained as a compatibility command')
    expect(mTaskDraftDoc).toContain('The existing `task:assign` permission is replaced by M-Task permissions')
