...
- **Active event contract coverage** = there is a real `publish()` call or literal durable outbox event intent in `apps/core/src/` or `services/*/src/`.
...
M-Net closed-loop subjects are active through durable event intents committed with their authoritative facts and dispatched through the production EventBus adapter.
...
| `mnet.join.requested.v0` | active closed-loop management subject | `MNetPendingJoinRequestSchema` |
| `mnet.join.approved.v0` | active closed-loop management subject | `MNetJoinApprovalGrantedSchema` |
| `mnet.join.rejected.v0` | active closed-loop management subject | `MNetJoinApprovalRejectedSchema` |
| `mnet.credential.issued.v0` | active closed-loop management subject | `MNetCredentialLifecycleResultSchema` |
| `mnet.credential.rotated.v0` | active closed-loop management subject | `MNetCredentialLifecycleResultSchema` |
| `mnet.credential.revoked.v0` | active closed-loop management subject | `MNetCredentialLifecycleResultSchema` |
| `mnet.topology.view.updated.v0` | active closed-loop management subject | `MNetTopologyViewSchema` |
| `mnet.topology.map.status.v0` | active closed-loop management subject | `MNetSignedTopologyMapStatusSchema` |
| `mnet.tunnel.health.v0` | active closed-loop management subject | `MNetTunnelHealthSchema` |
| `mnet.relay_policy.changed.v0` | active closed-loop management subject | `MNetForcedRelayPolicyResultSchema` |
| `mnet.profile.migration.changed.v0` | active closed-loop management subject | `MNetProfileMigrationResultSchema` |
| `mnet.break_glass.changed.v0` | active closed-loop management subject | `MNetBreakGlassGrantSchema` |
| `mnet.sidecar.degraded.v0` | active closed-loop management subject | `MNetSidecarStatusSchema` |
