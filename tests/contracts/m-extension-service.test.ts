...
} from '../../services/m-extension/src/app.ts'
    ...
    policy: {
      async authorize(_actor: ActorId, _action: Permission, _resource: string) {
        return {
          ...
          reasons: result === 'allow' ? ['permission_present'] : ['missing_permission']
        }
      }
    },
    log: {
      async writeTimeline(summary) {
        captured.timeline.push(summary)
      },
      async writeFull(_level, message, _correlationId, payload) {
        captured.full.push({ message, payload })
      },
      async writeAudit(actor, action, resource, resultValue, correlationId, payload) {
        captured.audits.push({
          actor,
          action,
          resource,
          result: resultValue,
...
describe('M-Extension external REST routes', () => {
  it('registers, lists, shows, enables, and disables a system/default extension', async () => {
    ...
    const showResponse = await app.handle(
      new Request(`http://localhost${mExtensionApiRoutes.collection}/extension-metadata-demo`, {
        headers: headers(adminToken)
      })
    )
    ...
    const enableResponse = await app.handle(
      new Request(
        `http://localhost${mExtensionApiRoutes.collection}/extension-metadata-demo/enable`,
        {
          method: 'POST',
          headers: headers(adminToken),
    ...
    const disableResponse = await app.handle(
      new Request(
        `http://localhost${mExtensionApiRoutes.collection}/extension-metadata-demo/disable`,
        {
          method: 'POST',
          headers: headers(adminToken),
    ...
    expect(captured.timeline).toEqual([
      'registered extension extension-metadata-demo',
      'enabled extension extension-metadata-demo',
      'disabled extension extension-metadata-demo'
    ])
  })
  ...
  it('fails closed on missing token and unsupported scope', async () => {
    ...
    const badScope = await app.handle(
      new Request(
        `http://localhost${mExtensionApiRoutes.collection}/extension-metadata-demo/enable`,
        {
          method: 'POST',
  ...
  it('re-registering an enabled extension does not implicitly disable the existing instance', async () => {
    ...
    const registeredAgain = await app.handle(
      new Request(`http://localhost${mExtensionApiRoutes.register}`, {
        method: 'POST',
        headers: headers(adminToken),
        body: JSON.stringify({ manifest: { ...manifest, displayName: 'Renamed Demo' } })
      })
    )
  ...
  it('reports internal readiness through the shared internal token boundary', async () => {
    ...
    const unauthorized = await app.handle(
      new Request(`http://localhost${mExtensionApiRoutes.ready}`)
    )
    ...
    const ready = await app.handle(
      new Request(`http://localhost${mExtensionApiRoutes.ready}`, {
        headers: { [internalTokenHeaderName]: 'm-extension-ready-token' }
      })
    )
