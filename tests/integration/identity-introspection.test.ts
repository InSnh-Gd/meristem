...
//
// Currently RED: the identity tables and Core identity routes do not exist
// yet. Tests are skipped when PostgreSQL is unavailable.
// ---------------------------------------------------------------------------
...
test.skipIf(!pgAvailable)('identity token issue → revoke → introspect lifecycle against PostgreSQL', async () => {
  // When Phase 17 creates the actors, actor_tokens, and actor_token_revocations
  // tables with their migration, this test will verify the full lifecycle.
  //
  ...
  try {
    ...
    // FAILS RED: table 'actors' does not exist yet
    ...
    // ── Step 2: Integration would issue a token via Core HTTP here ──────
    // In Phase 17: POST /api/v0/identity/tokens with security-admin auth
    // The Core HTTP server needs to be running for the full integration.
    ...
    // FAILS RED: table 'actor_tokens' does not exist yet
    ...
    // FAILS RED: table 'actor_token_revocations' does not exist yet
...
test.skipIf(!pgAvailable)('actor_tokens table has the correct identity v0.2 schema', async () => {
  const client = createSqlClient()
  ...
  try {
    ...
    // FAILS RED: table does not exist yet
...
test.skipIf(!pgAvailable)('actor_token_revocations table has correct schema', async () => {
  const client = createSqlClient()
