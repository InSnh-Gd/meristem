...
import { extensionDefinitions, extensionInstances, extensionTransitions, policyDecisions } from '../../packages/db/src/schema.ts'
...
describe('integration: m-extension PostgreSQL store', () => {
  test.skipIf(!pgAvailable)('persists definitions, instances, and transitions in authoritative tables', async () => {
    ...
    try {
      ...
      const registered = await store.register({ manifest, actor: 'admin', policyDecisionId: decisionId, correlationId: 'corr-extension-db' })
      ...
      const enabled = await reloaded.enable({ extensionId: manifest.id, actor: 'admin', policyDecisionId: decisionId, correlationId: 'corr-extension-db-enable' })
      expect(enabled?.instance.status).toBe('enabled')
      expect((await reloaded.transitions()).length).toBeGreaterThanOrEqual(2)
    } finally {
      await db.delete(extensionTransitions).where(eq(extensionTransitions.extensionId, manifest.id))
