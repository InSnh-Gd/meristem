...
禁止超大单体文件。代码必须按职责、功能域和边界合理拆分。

规则：

- **单文件行数上限**：非表格数据、非自动生成文件不得超过 500 行。超过上限必须在 commit message 中说明原因，并记录拆分计划。
- **单一职责**：每个文件只承担一个明确的功能或职责边界。禁止将路由、业务逻辑、存储适配、类型定义、错误处理混入同一文件。
- **目录反映模块边界**：目录结构必须反映功能域、服务、路由、组件或工具边界。禁止在单个目录下堆积无结构的散文件。
- **合理拆分粒度**：拆分不是为了拆而拆。共享类型、小型常量、紧密耦合的内部辅助函数可以共存于同一文件，但必须保持在行数上限内。
- **禁止 god-file**：禁止出现类似 `app.ts`、`index.ts` 包含所有路由、所有业务逻辑、所有适配器、所有类型的超大文件。入口文件只做组装和导出。

### 8.3 文件与测试命名

文件名和测试文件名必须准确反映其承担的功能与职责，禁止随意命名。

规则：

- **源文件命名**：文件名必须反映其唯一职责。示例：
  - `task-state-machine.ts`（状态机逻辑），不是 `utils.ts` 或 `helpers.ts`
  - `auth-middleware.ts`（认证中间件），不是 `middleware.ts`
  - `projection-cursor.ts`（投影游标），不是 `cursor.ts`
