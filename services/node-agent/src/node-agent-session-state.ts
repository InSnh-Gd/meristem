...
import { deriveControlUrl } from './node-agent-runtime-client.ts'

/**
 * Node agent 会话纯状态机：状态类型、事件、状态迁移、退避与心跳调度计算。
 * 模块内不做任何 I/O；网络结果由调用方解析后以事件形式喂给 transitionSessionState。
 */
