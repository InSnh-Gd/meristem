import { swagger } from '@elysiajs/swagger'
import { Elysia } from 'elysia'
import { createNodeControlRoutes } from './agent/node-control-routes.ts'
import { createNodeRuntimeRoutes } from './agent/node-runtime-routes.ts'
import { createClosedLoopRoutes } from './closed-loop/closed-loop-routes.ts'
import type { MNetAppDeps } from './deps.ts'
import { createForcedRelayRoutes } from './forced-relay/forced-relay-routes.ts'
import { createInternalRoutes } from './internal-routes.ts'
import { createOperationalRoutes } from './operational-routes.ts'
import { createGlobalDefaultsRoutes } from './profile/global-defaults-routes.ts'
import { createProfileAdminRoutes } from './profile/profile-admin-routes.ts'
import { createProfileRoutes } from './profile/profile-routes.ts'
import { createReadyRoute } from './ready-route.ts'

export type { MNetAppDeps } from './deps.ts'
export type { MNetServiceError, MNetServiceResult } from './types.ts'

export function createMNetApp(deps: MNetAppDeps) {
  return new Elysia()
    .use(
      swagger({
        path: '/api/v0/openapi',
        documentation: { info: { title: 'M-Net API', version: '0.1.0' } }
      })
    )
    .get('/health', () => ({ ok: true as const, service: 'm-net' as const }))
    .use(createReadyRoute(deps))
    .use(createInternalRoutes(deps))
    .use(createNodeControlRoutes(deps))
    .use(createOperationalRoutes(deps))
    .use(createNodeRuntimeRoutes(deps))
    .use(createProfileAdminRoutes(deps))
    .use(createProfileRoutes(deps))
    .use(createForcedRelayRoutes(deps))
    .use(createGlobalDefaultsRoutes(deps))
    .use(createClosedLoopRoutes(deps))
}

export type MNetApp = ReturnType<typeof createMNetApp>
