...
这些工具是开发辅助，不替代 `bun run lint` 中的既有边界导入检查。Agent 在回答结构性问题时应**优先使用 CodeGraph**，而不是重复发起大量 grep/read 探索。语义符号、调用方、调用路径与影响分析使用 CodeGraph；精确的语法结构证据使用 ast-grep CLI；在广域修改代码前，先用两者交叉验证证据。
