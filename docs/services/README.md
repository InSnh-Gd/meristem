...
| M-Extension | `m-extension.md` | Phase 15 supplemental extension control plane |
...
For the current local phases, Core may orchestrate logical node and logical network flows, but it must not implement real networking or bypass M-Policy / M-Log. After Phase 11, M-Task owns canonical noop task submission, lifecycle state, events, and task log facts. After Phase 15, M-Extension owns extension registry and instance state; Core must not own extension authoritative state.
