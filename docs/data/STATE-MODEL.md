...
The MVP concrete schema is defined in `docs/data/POSTGRES-SCHEMA.md`.
...
### 11.2 M-Net Closed-Loop Durable State Ownership

M-Net stores closed-loop join, credential, signed-map, key-status, tunnel-health, sidecar, relay-policy, profile-migration, and break-glass facts in PostgreSQL. Each authoritative mutation commits its fact updates and `event-intent` rows together in `mnet_closed_loop_facts`; EventBus failure therefore leaves a typed pending publication rather than an untracked half-completed mutation. Production startup retries pending intents at least once without replaying secret-provider or profile-migration side effects.

Stored payload decoding is fail-closed. A present row that does not decode against its versioned Effect Schema is a typed PostgreSQL decode failure, not an absent fact. OpenSearch and topology UI projections remain non-authoritative.
...
| **Network** | PostgreSQL（M-Net）: 网络拓扑、profile 状态、membership、closed-loop facts/outbox、relay 选择、key metadata | M-Net: 网络生命周期、profile 启用/禁用、topology 发布、node-runtime tunnel-health ingestion；Core: 网络创建、membership 注册入口 | M-UI BFF 通过 Core 边界读取网络状态；OpenSearch 可投影拓扑历史但非权威源 | NATS KV: 网络 map 缓存（signed map TTL 内有效，过期后 fail-closed） | NetBird sidecar 崩溃或 control-channel partition 驱动 typed degraded / fail_closed；EventBus 不可用保留 pending outbox 并重试；网络 map 过期后 fail-closed；中继不可达时仅在有可用 direct path 且 policy 允许时降级 | join decision、credential、relay、migration、break-glass 和 key lifecycle 高风险操作写 Audit before mutation；Audit 不可用阻塞操作 | M-Net |
