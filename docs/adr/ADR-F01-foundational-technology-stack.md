...
TypeScript 是 Meristem 的默认实现语言，覆盖 Core、核心功能域服务、契约、测试、CLI 和 UI 工具链。Wasm、Zig 或其他运行时仅在性能关键路径、隔离需求、低资源节点或特殊运行时能力场景下作为补充使用。
...
- 使用 `Effect.Service` / `Layer` 为生产、测试或功能域服务部署拥有不同实现的端口提供统一入口。
