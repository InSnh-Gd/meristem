...
import type { HostCapabilities, LiveProofVerdict, MNetV02LiveProofReport, PacketReachabilityEvidence, ProbeKind, ProofResult, StartedNodeAgent } from './mnet-v02-live-proof-contract.ts'
...
type LiveProofDeps = {
  ...
  readonly startNodeAgent: (input: { host: 'node-a' | 'node-b'; deploy: DeployProofReport; joinTicket: string }) => Promise<StartedNodeAgent>
  readonly readOverlayIp: (host: 'node-a' | 'node-b', nodeId: string) => Promise<string | null>
  readonly probeNetBirdHealth: (host: 'node-a' | 'node-b') => Promise<{ ok: boolean; detail: string }>
  readonly probePacketReachability: (input: { probe: ProbeKind; targetOverlayIp: string }) => Promise<{ ok: boolean; detail: string }>
...
function prerequisiteMissing(step: string, code: string, message: string, detail?: string): ProofResult {
...
function finalizeReport(input: {
  ...
  return {
    ...
    keycloakTokenVerification: input.keycloakTokenVerification ?? { status: 'not-run', detail: 'not reached' },
...
async function detectHostCapabilities(): Promise<HostCapabilities> {
  return {
    ...
    tcpProbe: commandExists(['nc', '-h']) || commandExists(['bash', '-lc', 'command -v timeout >/dev/null']),
...
function capabilityFailures(capabilities: HostCapabilities): ProofResult[] {
  ...
  if (!capabilities.docker) {
    failures.push(prerequisiteMissing('host.docker', 'host.docker_missing', 'Docker is required to start Keycloak and local supporting services'))
  }
  if (!capabilities.netbird) {
    failures.push(prerequisiteMissing('host.netbird', 'host.netbird_missing', 'netbird client binary is required on node-a and node-b'))
  }
  if (!capabilities.netAdmin) {
    failures.push(prerequisiteMissing('host.net_admin', 'host.net_admin_missing', 'CAP_NET_ADMIN is required for NetBird/WireGuard packet proof'))
  }
  if (!capabilities.tcpProbe && !capabilities.icmpProbe) {
    failures.push(prerequisiteMissing('host.packet-probe', 'host.packet_probe_missing', 'TCP or ICMP probe command is required to prove packet reachability'))
  }
  return failures
}
...
async function createProofNetwork(deps: LiveProofDeps) {
  ...
  const networkBody = (await networkResponse.json().catch(() => null)) as { network?: { id?: unknown } } | null
...
async function joinProofNetwork(
  ...
  const response = await deps.fetch(`http://127.0.0.1:3104/internal/v0/networks/${networkId}/members`, {
    method: 'POST',
    headers: {
      'content-type': 'application/json',
      'x-meristem-internal-token': process.env.MERISTEM_INTERNAL_TOKEN ?? 'deploy-proof-internal-token'
    },
...
async function enableProofProfile(deps: LiveProofDeps, networkId: string, token: string) {
  ...
  const enableResponse = await deps.fetch(`${serviceUrl}/api/v0/networks/${networkId}/profile`, {
    ...
    body: JSON.stringify({ profileVersion: 'm-net@0.3.0', reason: 'v0.2 live NetBird packet proof' })
  })
  const enableBody = (await enableResponse.json().catch(() => null)) as { status?: unknown; mapVersion?: unknown } | null
  if (!enableResponse.ok || enableBody?.status !== 'enabled') {
    return { ok: false as const, detail: `profile enable failed: HTTP ${enableResponse.status}`, networkId }
  }
  return { ok: true as const, detail: `enabled mapVersion=${String(enableBody.mapVersion)}`, networkId }
}
...
async function startNodeAgent(input: { host: 'node-a' | 'node-b'; deploy: DeployProofReport; joinTicket: string }): Promise<StartedNodeAgent> {
...
async function readOverlayIp(host: 'node-a' | 'node-b', nodeId: string): Promise<string | null> {
  const status = Bun.spawnSync([process.env.MERISTEM_NETBIRD_BINARY_PATH ?? 'netbird', 'status', '--json'], {
    cwd: rootDir,
    env: process.env,
    stdout: 'pipe',
    stderr: 'pipe'
...
async function probeNetBirdHealth(host: 'node-a' | 'node-b'): Promise<{ ok: boolean; detail: string }> {
  ...
  return {
    ok: result.exitCode === 0 && (normalized.includes('connected') || normalized.includes('running')),
    detail: `${host}: ${detail || `netbird status exited ${result.exitCode}`}`
  }
}

async function probePacketReachability(input: { probe: ProbeKind; targetOverlayIp: string }): Promise<{ ok: boolean; detail: string }> {
  const command = input.probe === 'tcp'
  ...
  const detail = result.stdout.toString().trim() || result.stderr.toString().trim() || `exit ${result.exitCode}`
...
const defaultDeps: LiveProofDeps = {
  ...
  runDeployProof: () => runV02DeployProof({ argv: ['bun', 'scripts/v02-deploy-proof.ts', '--target=nixos', '--auth=oidc'] }),
  fetch,
  mintOperatorToken,
  startNodeAgent,
  readOverlayIp,
...
export async function runMNetV02LiveProof(
  ...
  if (requestedTopology !== 'three-host') {
    results.push(failure('args.topology', 'topology.unsupported', 'live proof only supports --topology=three-host'))
  }
  if (requestedOidc !== 'keycloak') {
    results.push(failure('args.oidc', 'oidc.unsupported', 'live proof only supports --oidc=keycloak'))
  }
  ...
  results.push(success('host.capabilities', 'Docker, netbird, NET_ADMIN, and packet probe are available'))

  const deployProof = await deps.runDeployProof()
  if (deployProof.verdict !== 'pass') {
    const status = deployProof.verdict === 'prerequisite-missing' ? 'prerequisite-missing' : 'failure'
    const result = status === 'prerequisite-missing'
      ? prerequisiteMissing('deploy-proof', 'deploy.prerequisite_missing', 'v0.2 deploy proof did not reach pass')
  ...
  try {
    ...
    return finalizeReport({
      ...
      results: [...results, failure('keycloak.token', 'keycloak.token_failed', 'Keycloak operator token verification failed', detail)]
    })
  }
  const keycloakTokenVerification = { status: 'success' as const, detail: `minted token length=${token.length}` }
  ...
  if (!network.ok) {
    return finalizeReport({
      ...
      results: [...results, failure('m-net.network.create', 'mnet.network_create_failed', network.detail)]
    })
  }
  ...
  try {
    ...
    startedAgents.push(await deps.startNodeAgent({ host: 'node-a', deploy: deployProof, joinTicket: nodeAJoinTicket }))
    startedAgents.push(await deps.startNodeAgent({ host: 'node-b', deploy: deployProof, joinTicket: nodeBJoinTicket }))
  } catch (error) {
  ...
  for (const agent of startedAgents) {
    ...
    if (!member.ok) {
      ...
      return finalizeReport({
        ...
        results: [...results, failure('m-net.network.member', 'mnet.network_member_join_failed', member.detail)]
      })
    }
  ...
  if (!profile.ok) {
    ...
    return finalizeReport({
      ...
      results: [...results, failure('m-net.profile.enable', 'mnet.profile_enable_failed', profile.detail)]
    })
  }
  const profileEnable = { status: 'success' as const, detail: profile.detail, networkId: profile.networkId }
  ...
  if (health.some(probe => !probe.ok)) {
    ...
    return finalizeReport({
      ...
      results: [...results, failure('netbird.health', 'netbird.process_unhealthy', 'NetBird client health check failed')]
    })
  }
  ...
  if (!nodeBOverlayIp) {
    ...
    return finalizeReport({
      ...
      nodeAgentJoin,
      netbirdProcessHealth,
      packetReachability: { status: 'failure', source: 'node-a', target: 'node-b', detail: 'node-b overlay IP was not available from runtime evidence' },
      results: [...results, failure('packet.overlay-ip', 'packet.target_overlay_ip_missing', 'node-b overlay IP was not available')]
    })
  }
  ...
  const packetReachability: PacketReachabilityEvidence = packet.ok
    ? { status: 'success', probe, source: 'node-a', target: 'node-b', targetOverlayIp: nodeBOverlayIp, detail: packet.detail }
    : { status: 'failure', probe, source: 'node-a', target: 'node-b', targetOverlayIp: nodeBOverlayIp, detail: packet.detail }
  results.push(
    packet.ok
      ? success('packet.reachability', `${probe} probe reached ${nodeBOverlayIp}`)
      : failure('packet.reachability', 'packet.probe_failed', 'node-a could not reach node-b overlay IP', packet.detail)
  )
...
if (import.meta.main) {
  ...
  writeFileSync(join(rootDir, 'tests', 'evidence', 'mnet-v02-live-proof.json'), `${JSON.stringify(report, null, 2)}\n`)
