...
function deployResponse(
  ...
  return protectedResponse(success, { 400: apiErrorSchema, 404: apiErrorSchema, 503: apiErrorSchema, ...extra })
}
    ...
    new Elysia()
      ...
      .get(
        ...
        async ({ params, headers }) => {
          ...
          if (result.value === null) {
            missingFacadeResource(result.correlationId, 'deploy.proposal_not_found', 'proposal not found')
          }
          return result.value
        },
        {
      ...
      .post(
        ...
        async ({ headers }) =>
          runWrite(
            headers,
            mDeployFacadePermissions.driftRead,
            'deploy:drift',
            (_auth, ctx) => mDeploy.driftCheck(ctx)
          ),
        {
