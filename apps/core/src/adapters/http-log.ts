...
type SearchClient = {
  full: { get(params: { $query: Record<string, string> }): Promise<{ data: LogSearchResult<FullLog> | null; error: { value: unknown; status: number } | null }> }
  timeline: { get(params: { $query: Record<string, string> }): Promise<{ data: LogSearchResult<TimelineLog> | null; error: { value: unknown; status: number } | null }> }
  audit: { get(params: { $query: Record<string, string> }): Promise<{ data: LogSearchResult<AuditLog> | null; error: { value: unknown; status: number } | null }> }
}
...
export function createHttpLogPort() {
  ...
  return {
    ...
    async listAudit(limit?: number) {
      return runServiceEffect(
        tryServiceCall(() => client.internal.v0.audit.get({ $query: limit === undefined ? {} : { limit } }), { code: 'log.unavailable', message: 'Audit Log unavailable' }).pipe(
          Effect.flatMap((response) => requireServiceData(response, { code: 'log.unavailable', message: 'Audit Log unavailable' })),
          Effect.map((data) => data.entries)
        )
      )
    },
    async searchFull(query: FullLogSearchQuery) {
      return runServiceEffect(
        tryServiceCall(async () => {
          ...
          const response = await (client.internal.v0.search as SearchClient).full.get({ $query: params })
          if (response.error || !response.data) throw { code: 'search.unavailable', message: 'search unavailable' }
          return response.data as LogSearchResult<FullLog>
        }, { code: 'search.unavailable', message: 'search unavailable' }).pipe(Effect.map((data) => data))
      )
    },
    async searchTimeline(query: TimelineSearchQuery) {
      return runServiceEffect(
        tryServiceCall(async () => {
          ...
          if (query.correlationId) params.correlationId = query.correlationId
          const response = await (client.internal.v0.search as SearchClient).timeline.get({ $query: params })
          if (response.error || !response.data) throw { code: 'search.unavailable', message: 'search unavailable' }
    ...
    async searchAudit(query: AuditSearchQuery) {
      return runServiceEffect(
        tryServiceCall(async () => {
          ...
          if (query.correlationId) params.correlationId = query.correlationId
          const response = await (client.internal.v0.search as SearchClient).audit.get({ $query: params })
          if (response.error || !response.data) throw { code: 'search.unavailable', message: 'search unavailable' }
