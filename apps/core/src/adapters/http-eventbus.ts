...
export function createHttpEventPort() {
  const client = edenTreaty<EventBusApp>(serviceUrl('m-eventbus'), {
  ...
  return {
    async publish(subject: string, event: Parameters<CoreDeps['events']['publish']>[1]) {
      return runServiceEffect(
        tryServiceCall(() => client.internal.v0.publish.post({ subject, event }), {
          code: 'eventbus.unavailable',
          message: 'M-EventBus unavailable'
        }).pipe(
