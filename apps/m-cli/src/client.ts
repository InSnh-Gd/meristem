...
export function createCoreClient(config: CliConfig): CliClient {
  ...
  return {
    ...
    // 密钥客户端方法通过 Core 的 secret 控制面 API 操作密钥生命周期。
    secret: {
      async list(): Promise<Array<{ id: string; name: string; scope: string; status: string; createdBy: string; createdAt: string }>> {
        const result = await coreRoutes.getJson('/api/v0/secrets')
        if (!result.ok) throw new Error(result.error.message)
        return result.value as Array<{ id: string; name: string; scope: string; status: string; createdBy: string; createdAt: string }>
      },
      async get(id: string): Promise<{ id: string; name: string; scope: string; status: string; createdBy: string; createdAt: string; updatedAt: string; metadata: Record<string, string> }> {
        const result = await coreRoutes.getJson(`/api/v0/secrets/${id}`)
        if (!result.ok) throw new Error(result.error.message)
        return result.value as { id: string; name: string; scope: string; status: string; createdBy: string; createdAt: string; updatedAt: string; metadata: Record<string, string> }
      },
      async create(input: { name: string; scope: string; value: string; metadata?: Record<string, string> }): Promise<{ id: string; name: string; status: string; createdAt: string }> {
      ...
      async rotate(id: string, input: { value: string; reason: string }): Promise<{ id: string; version: string; status: string; rotatedAt: string }> {
      ...
      async disable(id: string, input: { reason: string }): Promise<{ id: string; status: string; disabledAt: string }> {
